...
#game variables
game_over = False
game_outcome = 0 # -1 is a loss & 1 is a win & 0 is no loss or win
level_started = False
...
for x in range(1, c.TURRET_LEVELS + 1):
    turret_sheet = pg.image.load(f'img/turret_{x}.png').convert_alpha()
    turret_spritesheets.append(turret_sheet)
#individual turret image for mouse cursor
cursor_turret = pg.image.load('img/cursor_turret.png').convert_alpha()
#enemies
enemy_images = {
    "weak": pg.image.load('img/enemy_1.png').convert_alpha(),
    "medium": pg.image.load('img/enemy_2.png').convert_alpha(),
    "strong": pg.image.load('img/enemy_3.png').convert_alpha(),
    "elite": pg.image.load('img/enemy_4.png').convert_alpha()
}
#buttons
buy_turret_image = pg.image.load('img/buy_turret.png').convert_alpha()
cancel_image = pg.image.load('img/cancel.png').convert_alpha()
upgrade_turret_image = pg.image.load('img/upgrade_turret.png').convert_alpha()
begin_image = pg.image.load('img/begin.png').convert_alpha()
restart_image = pg.image.load('img/restart.png').convert_alpha()
fast_forward_image = pg.image.load('img/fast_forward.png').convert_alpha()
#gui
heart_image = pg.image.load("img/gui/heart.png").convert_alpha()
coin_image = pg.image.load("img/gui/coin.png").convert_alpha()
logo_image = pg.image.load("img/gui/logo.png").convert_alpha()

#load sounds
shot_fx = pg.mixer.Sound("audio/audio_shot.wav")
...
# URL of the Google Font (Anton)
font_url = "https://fonts.gstatic.com/s/anton/v17/1Ptgg87LROyAm3Kz-C8.woff2"
font_path = "anton.woff2"
...
text_font = pygame.freetype.Font(font_path, 24)
large_font = pygame.freetype.Font(font_path, 36)

# Function for outputting text onto the screen
def draw_text(text, font, text_col, x, y):
    font.render_to(screen, (x, y), text, text_col)
...
while run:
    ...
    if game_over == False:
        ...
        if selected_turret:
            #show cost of upgrade and draw button
        ...
        if game_outcome == -1:
            draw_text("GAME OVER", large_font, "grey0", 324, 230)
        elif game_outcome == 1:
            draw_text("YOU WIN!", large_font, "grey0", 335, 230)
        # Restart level
